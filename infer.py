...
def infer(
    ...
    model = CCFMUNet(
        dim=(2, 128, 128), num_channels=32, out_channels=1, num_res_blocks=1
    ).to(device)
    ...
    for folder in folders:
        ...
        for image_file in tqdm(image_files, desc=f"{folder}", unit="image"):
            ...
            noisy_norm = normalize(raw[1:2], subset, channel=1)
            ...
            prediction = denormalize(full[:, :, 0], subset, channel=0)
